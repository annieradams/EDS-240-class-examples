...
jobs_clean %>% 
  ...
  slice_max(order_by = total_earnings, n = 10) %>% 
  pivot_longer(col = c(total_earnings_male, total_earnings_female), 
               names_to = "group",
...
jobs_clean %>% 
  filter(year == 2016) %>% 
  slice_max(order_by = total_earnings, n = 10) %>%  # take 10 highest occupation earnings
  ggplot(aes(x = fct_reorder(occupation, total_earnings), y = total_earnings)) + 
  scale_y_continuous(labels = scales::label_currency(accuracy = 1, scale = 0.001, suffix = "k"),limits = c(0, 250000))+
  ggalt::geom_lollipop()+
